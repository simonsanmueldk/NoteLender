        ...
        public async Task<Notification> DeleteGroup(string s)
        {
            ...
            try
            {
                var reply = await client.DeleteGroupAsync(
                    new Request {Name = s});
                Console.WriteLine("Greeting: " + reply.Message);
                return new Notification("Success", "Group "+s+" was successfully created. " , NotificationType.Success);
            }
            catch (RpcException e)
            {
                Console.WriteLine(e.Status.Detail);
                Console.WriteLine(e.Status.StatusCode);
                Console.WriteLine((int) e.Status.StatusCode);
                return new Notification(e.Status.StatusCode.ToString(), e.Status.Detail, NotificationType.Error);
            }
        }
